...
public class ErrorHandlerUser {
    ...
    public ru.practicum.shareit.item.controller.error.ErrorResponse handlerDuplicateException(final DuplicateException exception) {
        log.error("содержит дубликат {}", exception.getMessage());
        return new ru.practicum.shareit.item.controller.error.ErrorResponse(exception.getMessage());
    }
